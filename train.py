...
# data
dataset = 'wikitext-103-raw-v1/wikitext-103-raw'
...
# mup settings
mup_enabled =   True # Whether to use muP. If False then all other mup variables are ignored
...
mup_enable_coord_check_logging = True # If True will track the output.abs().mean() of various layers throughout training
# seed
seed = 1337
# DDP settings
...
# -----------------------------------------------------------------------------
def coordinate_check(vocab_size, base_n_embd, check_lr):
    import numpy as np
    CHECK_B     = 2
    CHECK_T     = 64
    CHECK_STEPS = 10
    ...
    results = {}
    for width_mult in [1.0, 2.0]:
        check_n_embd = int(base_n_embd * width_mult)
        torch.manual_seed(0)
        check_args = dict(
            n_layer=n_layer, n_head=n_head, n_embd=check_n_embd,
            block_size=block_size, bias=bias, vocab_size=vocab_size,
            dropout=dropout, mup_enabled=True,
        ...
        m = GPT(GPTConfig(**check_args)).to(device)
        opt = m.configure_optimizers(weight_decay, check_lr, (beta1, beta2), device_type)

        torch.manual_seed(999)
        x_probe = torch.randint(0, vocab_size, (CHECK_B, CHECK_T), device=device)
        y_probe = torch.randint(0, vocab_size, (CHECK_B, CHECK_T), device=device)

        m.eval()
        with torch.no_grad():
            logits_0, _ = m(x_probe)
            logits_0 = logits_0.detach().float()

        m.train()
        for _ in range(CHECK_STEPS):
            _, loss = m(x_probe, y_probe)
            loss.backward()
        ...
        m.eval()
        with torch.no_grad():
            logits_t, _ = m(x_probe)
            coord_std = (logits_t.float() - logits_0).std().item()

        results[width_mult] = coord_std
        print(f"  width_mult={width_mult:.1f}  n_embd={check_n_embd}  |  std(delta_logit) after {CHECK_STEPS} steps = {coord_std:.5f}")
        del m, opt
        torch.cuda.empty_cache()

    ratio = results[2.0] / (results[1.0] + 1e-9)
    ok = 0.5 < ratio < 2.0
    status = "PASS" if ok else "FAIL — check muP scaling rules"
    print(f"  ratio (2x/1x width) = {ratio:.3f}  ->  {status}")
    print("-" * 66 + "\n")
    return ok
...
def get_batch(split):
    ...
    if split == 'train':
        data = np.memmap(os.path.join(data_dir, 'wiki.train.bin'), dtype=np.uint16, mode='r')
    else:
        data = np.memmap(os.path.join(data_dir, 'wiki.valid.bin'), dtype=np.uint16, mode='r')
...
if init_from == 'scratch':
    ...
    if mup_enabled and mup_enable_coord_check_logging:
        coordinate_check(model_args['vocab_size'], n_embd, learning_rate)
